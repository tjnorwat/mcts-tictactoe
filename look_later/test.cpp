...
struct Node {
    ...
    Node* select() {
        ...
        for (Node* child : this->children) {
            ...
            double ucb = q_value + 1.41 * sqrt(log(this->visits) / (double)child->visits);
    ...
    // expand needs to be random ? 
    Node* expand() {
        
        // if (this->fullyExpanded()) {
        //     return nullptr;  // No more children to expand
        // }
    ...
    int simulate() {
        ...
        while (!current_state.isTerminal()) {
            ...
            current_state = next_states[distribution(this->generator)];
        }
    ...
    void backpropagate(int result) {
        ...
        while (current != nullptr) {
            current->visits++;
            current->score += result;  // Assume result is 1 for win, 0 for draw, -1 for lose
            current = current->parent;  // Move to the parent node
        }
    }

    // if children size is equal to the number of possible moves 
...
struct MCTS {
    ...
    uint16_t runSearch() const {
        ...
        while (true) {
            ...
            if (node->fullyExpanded()) {
                ...
                Node *new_node = node->expand();
                int winner = new_node->simulate();
                new_node->backpropagate(winner);
            }
            iterations++;
        }
        cout << "Iterations: " << iterations << endl;
        return this->root->select()->state.move_idx;
    }

    uint16_t bestMove() const {
        if (!this->root->fullyExpanded()) {
            return -1;
        }
        ...
        Node temp;
        for (const Node* child : this->root->children) {
    ...
    uint16_t everything() {
        uint16_t move = this->runSearch();
        uint16_t best_move = this->bestMove();
